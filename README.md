...
(http://jinja.pocoo.org/docs/). It is based on a port of the Ocaml
...
`from-string :: STRING [env: TEMPLATE-ENVIRONMENT] [models: ALIST] [ctx: TEMPLATE-CONTEXT] -> STRING`
...
`from-file :: STRING [env: TEMPLATE-ENVIRONMENT] [models: ALIST] [ctx: TEMPLATE-CONTEXT] -> STRING`

Parses the given file and evaluates the resulting template statements
in the given context and models. See the next section and the data
types section for detailed definitions of the environment, context and
statement types.

---

`eval-expr :: TEMPLATE-ENVIRONMENT * TEMPLATE-CONTEXT * TEXPR -> TVALUE`
...
- `filters: STRING LIST` : a list of filters to be applied to the input
- `lexer-table: LEXER-TABLE` : a table generated by `make-lexer-table`

---

`init-context :: [env: TEMPLATE-ENVIRONMENT] [models: ALIST] [open-buffer: VOID -> PORT] -> TEMPLATE-CONTEXT`
...
`statements-from-string :: TEMPLATE-ENVIRONMENT * STRING -> TSTMT LIST`
...
`statements-from-file :: TEMPLATE-ENVIRONMENT * STRING -> TSTMT LIST`
...
- `frame-stack`  : a stack where each element is a list of bindings of names to template values
...
- `(ForStatement    (e1 texpr?) (e2 texpr?) (a template-ast?))` : executes the statements in `a` for the given range
...
loop.previtem / loop.nextitem — suppress repeated values in tabular output
=========================================================================

{% with report_rows = [{obj: "obj1", prop: "prop1", val: "val1"},{obj: "obj1", prop: "prop2", val: "val2"},{obj: "obj1", prop: "prop3", val: "val3"},{obj: "obj2", prop: "prop1", val: "val1"}] %}
{% for row in report_rows %}
{% if loop.first or row.obj != loop.previtem.obj %}{{row.obj}}, {% endif %}{{row.prop}}, {{row.val}}
{% endfor %}
{% endwith %}
...
- 1.27 : Added `loop.previtem` and `loop.nextitem`; fixed `op-sort` reverse/attribute kwargs; fixed `op-map` and `fun-or-attr`
