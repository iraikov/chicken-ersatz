...
(define (group-by foldf path seq)
  (let* ((keys (map string->symbol (string-split path ".")))
         (groups (foldf (lambda (x ax)
                          (let ((v (fold (lambda (key obj) (tobjval-lookup obj key)) x keys)))
...
(define (op-groupby pathval value kwargs)
  (let ((path (unbox-string pathval)))
    (cases tvalue value
           (Tlist (l) (group-by fold path l))
           (Tvector (v) (group-by vector-fold path v))
	   (else (error 'groupby "operand type error" value))
	   ))
  )
           
(define (op-map value fn kwargs)
    ...
    (let* ((defaults  `((reverse . ,(Tbool #f))
      ...
      (let ((lst (unbox-list lst)))
        (cases tvalue (car lst)
               (Tstr (s)
                     (Tlist (map Tstr (sort (cons s (map unbox-string (cdr lst)))
               ...
               (Tfloat (i)
                ...
                (Tobj (alst)
                      (if (string-null? attribute)
                          (error 'sort "empty attribute")
                          (let ((attr-sym (string->symbol attribute)))
                            (let* ((attr-val (tobjval-lookup (car lst) attr-sym))
                                   (result (cases tvalue attr-val
                                             (Tstr (s)
                                               (Tlist (sort lst (lambda (x y)
                                                                  (let ((x-val (unbox-string (tobjval-lookup x attr-sym)))
                                                                        (y-val (unbox-string (tobjval-lookup y attr-sym))))
                                                                    (if reverse
                                                                        (string> x-val y-val)
                                                                        (string< x-val y-val)))))))
                                             (Tint (i)
                                               (Tlist (sort lst (lambda (x y)
                                                                  (let ((x-val (unbox-int (tobjval-lookup x attr-sym)))
                                                                        (y-val (unbox-int (tobjval-lookup y attr-sym))))
                                                                    (if reverse
                                                                        (> x-val y-val)
                                                                        (< x-val y-val)))))))
                                             (Tfloat (f)
                                               (Tlist (sort lst (lambda (x y)
                                                                  (let ((x-val (unbox-float (tobjval-lookup x attr-sym)))
                                                                        (y-val (unbox-float (tobjval-lookup y attr-sym))))
                                                                    (if reverse
                                                                        (> x-val y-val)
                                                                        (< x-val y-val)))))))
                                             (else (error 'sort "unsupported type for attribute")))))
                              result))))
